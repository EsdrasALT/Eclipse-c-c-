...
DataNascimento solicitarDataDeNascimento() {
	...
	while (formato_incorreto) {
		...
		if (data.dia >= 1 && data.dia <= 31 && data.mes >= 1 && data.mes <= 12
				&& data.ano >= 1900 && data.ano <= 2024) {
			formato_incorreto = 0; // Saida do loop
		} else {
			printf(
					"Formato incorreto. Por favor, insira a data no formato correto.\n");
		}
	}
...
int verificarIdade(DataNascimento data_nascimento) {
	...
	if (data_atual.mes < data_nascimento.mes
			|| (data_atual.mes == data_nascimento.mes
					&& data_atual.dia < data_nascimento.dia)) {
...
void chamarFuncoesPlacas() {
	...
	while (placaInvalida) {
		printf(
				"Cadastro de placa modelo novo ou modelo antigo? (A - Antigo OU N - Novo)\n");
		scanf(" %c", &escolha);
...
void validarPlacaAntiga() {
	...
	do {
		printf(
				"Digite a placa seguindo o modelo antigo (exemplo: ABC1234): \n");
		scanf("%8s", digitacao);
		getchar();
		...
		for (int i = 0; i < 3; i++) {
			if (!isupper(digitacao[i])) { //== if (!(digitacao[i] >= 'A' && digitacao[i] <= 'Z')) {
				printf(
						"Padrão incorreto na posição %d (os três primeiros devem ser letras maiúsculas)\n",
						i + 1);
				valido = 0;
				break;
			}
		...
		for (int i = 3; i < PLACA_LENGTH; i++) {
			if (!isdigit(digitacao[i])) { // == if (!(digitacao[i] >= '0' && digitacao[i] <= '9')) {
				printf(
						"Padrão incorreto na posição %d (os quatro últimos devem ser dígitos)\n",
						i + 1);
				valido = 0;
				break;
			}
	...
	do {
		...
		for (int i = 5; i < PLACA_LENGTH; i++) {
			if (!isdigit(digitacao[i])) {
				printf(
						"Padrão incorreto na posição %d (os dois últimos devem ser dígitos)\n",
						i + 1);
				valido = 0;
				break;
			}
